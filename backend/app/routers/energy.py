...
Exposes both halves of the dual-energy backbone as standalone POSTs so
the frontend can preview "what does this system produce here?" using
either the pvlib chain or the manual physics chain without going
...
will land later in the plan; this router gives us testable slices now.
"""
...
from app.services import energy_manual, energy_pvlib, pvgis_service
...
@router.post("/manual", response_model=EnergyManualResult)
async def estimate_manual(request: EnergyManualRequest) -> EnergyManualResult:
    """Run the from-first-principles physics simulation for the given location.

    Internally fetches the same PVGIS TMY used by the pvlib endpoint
    and runs the manual chain documented in
    :mod:`app.services.energy_manual`. The two endpoints together form
    the dual-energy backbone of the thesis — disagreement between them
    is the model-uncertainty quantification.
    """
    try:
        tmy = await pvgis_service.fetch_tmy(
            request.location.latitude, request.location.longitude
    ...
    losses = (
        energy_manual.DEFAULT_SYSTEM_LOSSES_FRACTION
        if request.system_losses_fraction is None
        else request.system_losses_fraction
    )
    ...
    except energy_manual.EnergyModelError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc

    return EnergyManualResult(
        annual_kwh=sim.annual_kwh,
        monthly_kwh=sim.monthly_kwh,
        specific_yield_kwh_per_kwp=sim.specific_yield_kwh_per_kwp,
        capacity_factor=sim.capacity_factor,
