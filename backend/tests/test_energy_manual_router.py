"""Tests for the /api/energy/manual endpoint.

PVGIS is patched so the router never touches the network. Mirrors the
pvlib-router tests so any divergence in routing behaviour between the
two halves of the dual-energy backbone surfaces immediately.
"""
...
def test_manual_endpoint_happy_path(fake_tmy):
...
def test_manual_endpoint_honours_overrides(fake_tmy):
    with _patch_fetch_tmy(fake_tmy):
        response = client.post(
            "/api/energy/manual",
            json={
...
def test_manual_endpoint_502_on_pvgis_failure():
...
def test_manual_endpoint_validates_system_kw():
